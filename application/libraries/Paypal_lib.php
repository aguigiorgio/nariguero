...
class paypal_lib {
	...
	function paypal_auto_form()
	{
		...
		$this->button('Clic aquí si no es redirigido automáticamente.');

		echo '<html>' . "\n";
		echo '<head><title>Procesando Pago...</title></head>' . "\n";
		echo '<body style="text-align:center;font-family:Tahoma;margin-top:10%;" onLoad="document.forms[\'paypal_auto_form\'].submit();">' . "\n";
		echo '<img src="'.base_url().'images/ajax-loader.gif" />'."\n";
		echo '<p style="text-align:center;">Espere por favor, Su orden está siendo procesada y mandada al sitio de Paypal.</p>' . "\n";
	...
	function validate_ipn()
	{
		...
		else
		{
			...
			$this->last_error = 'IPN Validación fallida.';
			$this->log_ipn_results(false);
			return false;
		}
	}
	...
	function log_ipn_results($success)
	{
		...
		if ($success) $text .= "¡Exitoso!\n";
		else $text .= 'Fallo: '.$this->last_error."\n";

		// Log the POST variables
		$text .= "IPN Variables POST desde Paypal:\n";
		foreach ($this->ipn_data as $key=>$value)
			$text .= "$key=$value, ";

		// Log the response from the paypal server
		$text .= "\nIPN Respuesta desde el servidor de Paypal:\n ".$this->ipn_response;
